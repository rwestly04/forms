...
/* AUTO FORMAT + REAL-TIME VALIDATION */
...
function validateName(input) {
  ...
  if (input.value === "") { formGroup.classList.remove("error"); formGroup.querySelector(".error").textContent = ""; return; }
...
[firstName, middleName, lastName].forEach(input => {
  input.addEventListener("input", () => { formatName(input); validateName(input); });
});
...
password.addEventListener("input", () => {
  ...
  if (strength <= 1) { strengthBar.style.width="25%"; strengthBar.style.background="red"; strengthText.textContent="Weak"; }
  else if (strength <= 3) { strengthBar.style.width="60%"; strengthBar.style.background="orange"; strengthText.textContent="Medium"; }
  else { strengthBar.style.width="100%"; strengthBar.style.background="green"; strengthText.textContent="Strong"; }
});
...
form.addEventListener("submit", function(e){
  ...
  document.querySelectorAll(".form-group").forEach(group=>{ group.classList.remove("error"); group.querySelector(".error").textContent=""; });
  ...
  let genderSelected=false; gender.forEach(g=>{ if(g.checked) genderSelected=true; });
